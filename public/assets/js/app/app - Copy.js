...
app.controller('mushafController', function($scope,$sce, $http, API_URL, searchService, $cookieStore,ngAudio){
	...
	$scope.init = function() {
		...
		$scope.selectedListen = "AbdulSamad_64kbps_QuranExplorer.Com";
		$scope.sound = ngAudio.load("http://www.everyayah.com/data/"+$scope.selectedListen+"/"+$scope.activeMenu+".mp3"); // returns NgAudioObject
		$scope.allFehras();		
		$scope.allJuz();		
